...
The inference engine — runs the model on decrypted text; sees plaintext by role. Upstream third-party ([github.com/vllm-project/vllm](https://github.com/vllm-project/vllm)). Serves the Gemma-4, gpt-oss and Qwen3-VL models on near.ai; the sibling engine `lmsysorg/sglang` serves the DeepSeek/Qwen/GLM models.
...
**A defect fixed between two of these builds.** The gpt-oss page's crash-path finding — `CachedRequestData` lacking an `anon_repr`, so an engine-crash dump serialises its `all_token_ids` (full prompt **and** output token ids) through the `__dict__` fallback to stdout — is present at `v0.12.0` and **absent at `v0.17.0`**, where `CachedRequestData.anon_repr` redacts to `all_token_ids_lens`. Same file, same class, upstream fix in between. The two reviews were conducted independently and each verified its own tree; the divergence is real, not a disagreement.

**Version spread is wide.** These three builds span ~14 minor releases: gpt-oss runs the **v0.12.0** release (2025-12), Qwen3-VL an untagged **v0.17.0-line** build (2026-03), Gemma-4 a **2026-06 nightly**. Each page is verified against its own tree — no finding is carried across.
