...
a server page, and it currently always contains one item (§8).

---

## 1. Keys — the first axis, walked end to end
...
## 3. Renderers — where the client's one HIGH came from
...
## 4. The send gate and the seal — three pre-1.0 HIGHs
...
## 5. Entry points that are not the composer
...
## 6. Model-callable tools — content-driven egress
...
## 7. At rest — the store and every copy of it
...
## 8. On-device inference — the standing *not traced*
...
- [ ] Added 2026-09-10 — the binary's **network capability** is readable from
      the bytes: `otool -L` for linked frameworks, `nm -u` for socket imports,
      then `otool -tv` and a walk one hop above every `connect` /
...
## 11. Identity, before any of the above
...
read per hit, and it would have surfaced the image loader in seconds. Items 3,
