...
## verdict: QUALIFIED-PASS — private at the reviewed tree; one HIGH (a no-tap markdown-image auto-fetch in the transcript renderer, driven by assistant content) found and fixed before publish; residuals are by design — opt-in web search is content-driven egress to one fixed host, keys restore via encrypted backup, an unattested provider is plaintext by definition
...
- **Storage.** Keychain `kSecAttrAccessibleAfterFirstUnlock`, not
  synchronizable; the providers JSON holds the id. No key in any file, log
  line, or URL query.
