...
## verdict: QUALIFIED-PASS — keys: they leave the device only to their own provider, but running 1.0.2 later found the near.ai key at rest in cleartext in the shared URL cache, present here unchanged (MEDIUM, fixed in 1.0.3); messages: private at the reviewed tree, one HIGH (a no-tap markdown-image auto-fetch driven by assistant content) found and fixed before publish, and the user-edited system prompt in UserDefaults inside backups (LOW, found later); residuals are by design
...
# teemoon iPhone client (1.0) — Plaintext and Key Exfiltration Audit
...
### MEDIUM (deployed: yes; fixed in 1.0.3) — the near.ai API key is written in cleartext to the shared URL cache on disk
...
the fix and its measurement on the [1.0.3 page](v1.0.3-7edcb55.md#findings).
