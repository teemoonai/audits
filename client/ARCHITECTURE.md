...
flowchart TB
    ...
    classDef sink fill:#fecaca,stroke:#dc2626,color:#000
    class UI,ENGINE,E2EE,PLAIN,LOCAL,RENDER,SIRI,TOOLS,MN plaintext
    class GW ciphertext
    class STORE,LOG,PB,KEYS sink
```

**Yellow = holds your plaintext.** Everything inside the app that touches a
message is yellow by construction; the question is only whether it copies the
message to a **red** sink (a store, a log, the pasteboard, the Keychain) in a
form someone else can read, or to a network destination other than the one the
send path was aimed at. Blue is the ciphertext-only gateway from the server-side
map; the dotted lines are the app's own verification and download traffic,
which carries quotes, nonces, digests and weights — never a message.
