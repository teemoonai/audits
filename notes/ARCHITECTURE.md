...
|---|---|---|---|
| teemoon (device) | Logs / at-rest store / egress / renderer / URL cache | QUALIFIED-PASS (1 HIGH fixed pre-publish; 1 MEDIUM — the near.ai key at rest in the shared URL cache, found by running 1.0.2 — fixed in 1.0.3 and measured closed; 1 LOW: user-edited system prompt in UserDefaults) | A **no-tap markdown-image auto-fetch** in the transcript renderer, driven by assistant-authored content, could have exfiltrated reply-side plaintext over a URLSession outside the E2EE transport — found and fixed pre-publish (renderer `imageURL` strip + a non-fetching attachment loader). At-rest store **and** the FTS search sidecar are encrypted (`completeUnlessOpen`) + backup-excluded; no telemetry SDKs; key copy confined to a concealed, expiring pasteboard. The Siri/Shortcuts entry fails closed (`RequestLLMIntent` → `session.sendPolicy`, teemoon-ios `846dc5c`). Residual by design: an unattested provider is plaintext by definition. Pinned to App Store 1.0.3 (`7edcb55`); the device has its own map, method and per-release pages under [`/client/`](/client/ARCHITECTURE.md) | [teemoon iOS client](/notes/teemoon-ios-plaintext-audit.md) · [client map](/client/ARCHITECTURE.md) |
