...
| [`client_drift.py`](client_drift.py) | Does every App Store tag of teemoonai/teemoon-ios have a page under `client/teemoon-ios/` that pins the commit the tag points at? The client has no attestation, so the tag→sha binding is the identity. Exit 0 = every release audited. | `python3 tools/client_drift.py [--json]` — daily via `.github/workflows/client-drift.yml`, which files an issue on drift |
